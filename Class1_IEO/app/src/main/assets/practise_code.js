var pos = 0,  qnumber = 0, wrongpos=0,wrongcount=0,noattempt=0, wrongcountrepeat=0,test,CorrectAnswer, test_status, question, choice, choices, chA, chB, chC,chD, correct = 0;
...
function renderQuestion(){
	...
	if(pos >= questions.length){
		...
		test.innerHTML = "<h3 class='you-got'>You can click on 'Review' button to check wrongly attempted questions.<BR><span class='ans'>"+correct+"</span> of <span class='length'>"+questions.length+"</span> questions are correct.<BR>Test Completed in <U>" + timeTaken +" Minutes </U><br><br><BR>Number of wrong questions  = "+wrongcount+"</h3>";
		_("test_status").innerHTML = "<BR>Test Completed in 789 <U>" + timeTaken +" Minutes </U><br><br>";
...
var ul = document.getElementById('paging');
	...
	qnumber = parseInt(target.innerHTML);
	pos=qnumber-1;
	renderQuestion();
};
	...
	chA =toTitleCase(questions[pos][1]);
	chB = toTitleCase(questions[pos][2]);
	chC = toTitleCase(questions[pos][3]);
	chD = toTitleCase(questions[pos][4]);
...
var ID;
	...
	test = _("test");
	if(pos >= questions.length){
		test.innerHTML = "<h2>1     You got "+correct+" of "+questions.length+" questions correct.</h2>";

		_("test_status").innerHTML = "Test Completed 456 ";
		pos = 0;
		correct = 0;
...
table.setAttribute('border','1')
	...
	for(j=1; j<=4; j++){
	  ...
	  if (i==2)
	  {
	     ...
	     if(j==4)
		   var cellText = document.createTextNode("Unattempted");
	  }
	  ...
	  if (i==1)
	  {
		  ...
		  if(j==2)
		   var cellText = document.createTextNode(correct);
		  if(j==3)
		   var cellText = document.createTextNode(wrongcount);
	      if(j==4)
		   var cellText = document.createTextNode(wrong-wrongcount);
